...
Use the scope named in the invocation input; otherwise infer it from recent work. Map it to the matching `codex exec review` selector:

| Scope | Selector |
|---|---|
| Uncommitted work (staged, unstaged, untracked) | `--uncommitted` |
| Branch or PR diff | `--base origin/<default-branch>` |
| Single commit | `--commit <SHA>` |

For `--base`, fetch first (`git fetch origin <default-branch>`) and pass the remote-tracking ref (`origin/main`, not `main`). A fetch updates only the remote-tracking ref, so a local branch name can silently compare against a stale base. State the scope in your first sentence so the user can redirect.
...
Before launch, write run metadata: run ID, absolute workspace, requested scope, resolved base and head SHAs, staged and unstaged diff identities and relevant untracked path and content hashes, CLI version, command, requested model and effort, and start time. Exclude task-owned report artifacts from the requested review. For uncommitted work, inventory all relevant content; a HEAD SHA alone does not identify it. Use an isolated snapshot if concurrent writers cannot stop. Do not silently expand scope to unrelated changes.

Use the installed CLI's supported selector and prompt combination. If selector plus custom prompt is unsupported, use its built-in rubric for ordinary review; for a special requested focus, use a supported prompt-only invocation with the exact source scope. Describe which rubric was actually supplied. Preserve least privilege; do not bypass approvals or sandbox protections for the reviewer. The reviewer is a leaf: it must not dispatch a review of its own.
