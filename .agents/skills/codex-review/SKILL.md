...
3. Add the server per run with `-c` flags; nothing is written to `config.toml`:
   ...
   Exec mode's approval policy is `never`, so without `default_tools_approval_mode="approve"` every browser call fails with `MCP tool call requires approval, but approval policy is never`. The setting covers this one server only. Valid values: `auto`, `prompt`, `writes`, `approve`. `--output-dir` places only auto-named captures; a screenshot given a relative filename lands in the reviewed workspace and changes its state. Tell the reviewer to omit filenames or pass absolute paths inside the image directory.
