import { collectFacts, removedLine } from "./facts.mjs";
import { esc, listPhrase, plural, writeText } from "./lib.mjs";

const facts = collectFacts();
const marker = "<!-- generated by scripts/readme/build.mjs. do not edit by hand. -->";
// Name template skills that are not installed, so the README says why they are absent.
const header = facts.removed.length ? `${marker}
${removedLine(facts.removed)}` : marker;
...
function skillLists() {
  const sections = facts.groups.filter((group) => facts.tierCounts[group.id] > 0).map((group) => {
...
${picture("boot", `Harness Firmware boots with ${plural(facts.skillCount, "skill")}, ${plural(facts.referenceFileCount, "project-memory file")}, and ${plural(facts.runtimeCount, "runtime boundary", "runtime boundaries")} ready.`)}
...
## ${plural(facts.skillCount, "workflow")}, loaded when called
...
${picture("skills", `A memory map of ${plural(facts.skillCount, "on-demand workflow")} grouped into ${listPhrase(groupCounts)} ${facts.skillCount === 1 ? "skill" : "skills"}.`)}

</details>

<details>
<summary><strong>Click to browse ${facts.skillCount === 1 ? "the only skill" : `all ${facts.skillCount} skills`}</strong></summary>
