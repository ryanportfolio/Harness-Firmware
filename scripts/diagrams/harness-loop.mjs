...
const SKILL = (name) => `${TREE}/.claude/skills/${name}`;
...
function inputBox(y, title, lines, ref, href, icon) {
  const rows = lines.map((line, i) => `<text class="copy mute" x="16" y="${72 + i * 20}">${esc(line)}</text>`).join("");
  return `<a href="${href}"><g transform="translate(24 ${y})">
<rect class="panel" width="272" height="150" rx="8"/>
<g transform="translate(24 30)">${icon}</g>
<text class="subhead ink" x="52" y="36" font-weight="600">${esc(title)}</text>
${rows}
<text class="ref accent" x="16" y="132">${esc(ref)}</text>
</g></a>`;
}
...
function build(themeName) {
  ...
  const inputs = [
    ...
    inputBox(440, "Evaluators", [
      "CI, tests, audited evidence:",
      "evidence beats claims",
    ], ".github/workflows/ · /verify-this", `${TREE}/.github/workflows`,
    `<path class="glyph" d="M-9 4A9 9 0 0 1 9 4"/><path class="glyph" d="M0 4L5-3"/><path class="glyph" d="M-11 8H11"/>`),
  ];

  const inputArrows = [175, 345, 515]
    .map((y) => `<path class="wire" marker-end="url(#arrow)" d="M296 ${y}H316"/>`)
    .join("");

  const supervisor = `<a href="${SKILL("long-horizon")}"><g transform="translate(860 90)">
<rect class="panel" width="436" height="150" rx="8"/>
<text class="label mute" x="16" y="28">SUPERVISOR · STAGNATION WATCH</text>
...
<text class="ref accent" x="16" y="126">long-horizon (Stagnation) · codex-review (gpt-5.6-sol)</text>
</g></a>
...
<text class="tiny" fill="${warn}" x="872" y="262">conditional intervention</text>`;
  ...
  const candidate = `<a href="${SKILL("merge")}"><g transform="translate(860 350)">
<rect class="panel" width="170" height="120" rx="8"/>
<text class="subhead ink" x="16" y="32" font-weight="600">Candidate</text>
<text class="copy mute" x="16" y="58">branch + PR,</text>
<text class="copy mute" x="16" y="78">evidence attached</text>
<text class="ref accent" x="16" y="102">/merge cycle · gh pr</text>
</g></a>
...
<a href="${SKILL("refine")}"><g transform="translate(1138 340)">
<rect width="158" height="140" rx="8" fill="none" stroke="${theme.accent}" stroke-width="1.5"/>
<text class="subhead ink" x="12" y="30" font-weight="600">Updated lineage</text>
...
<text class="ref accent" x="12" y="122">refine · recall save</text>
</g></a>
<path class="warnwire dash" marker-end="url(#warnarrow)" d="M1085 450V600H835"/>
<a href="${SKILL("babysit-ci")}"><text class="tiny" fill="${warn}" x="960" y="590" text-anchor="middle">repair and retry · babysit-ci</text></a>`;
...
<text class="tiny accent" x="560" y="68">lessons merge back into the firmware: kernel, skills, reference</text>`;
  ...
  const footerChips = [
    { label: "arena · parallel candidates", href: SKILL("arena") },
    { label: "wow-loop · adversarial critique", href: SKILL("wow-loop") },
    { label: "impartial + Codex review", href: SKILL("impartial-review") },
    { label: "RTK/STK token diet", href: null },
    { label: "unslop + caveman prose gates", href: SKILL("unslop") },
    { label: "sync-starter · every repo", href: SKILL("sync-starter") },
  ];
  let chipX = 24;
  const chips = footerChips.map(({ label, href }) => {
    const width = 12 + label.length * 7.4;
    const body = `<g transform="translate(${fmt(chipX)} 690)"><rect class="cell" width="${fmt(width)}" height="34" rx="17"/><text class="small mute" x="${fmt(width / 2)}" y="21" text-anchor="middle">${esc(label)}</text></g>`;
    chipX += width + 14;
...
${inputs.join("")}
${inputArrows}
<rect class="panel" x="320" y="90" width="500" height="560" rx="10"/>
<a href="${SKILL("long-horizon")}"><text class="label mute" x="570" y="118" text-anchor="middle">AGENTIC ROUND LOOP · LONG-HORIZON + FABLE-MODE</text></a>
${loopArrows()}
${loopNodes()}
<circle cx="${CX}" cy="378" r="94" class="panel dash"/>
...
<text class="small mute" x="${CX}" y="396" text-anchor="middle">state file · subagents · tools</text>
<text class="tiny mute" x="${CX}" y="414" text-anchor="middle">contract · verified · remaining · dead ends</text>
${supervisor}
${candidate}
${refine}
<text class="label mute" x="24" y="678">ALSO IN THE FIRMWARE</text>
${chips}
<text class="small mute" x="1296" y="745" text-anchor="end">same loop shape AVO uses to reach 100 RHAE on ARC-AGI-3 · the harness, not the model, carries it</text>`;
...
.cell{fill:none;stroke:${theme.rule};stroke-width:1}
...
@media (prefers-reduced-motion:reduce){*{animation:none!important}}`;
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${W} ${H}" width="${W}" height="${H}" role="img" aria-label="Harness firmware architecture: inputs feed an audited round loop, a stagnation supervisor intervenes conditionally, candidates pass a CI gate into an updated lineage that feeds lessons back into the firmware.">
