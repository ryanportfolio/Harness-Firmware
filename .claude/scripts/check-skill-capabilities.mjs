...
import { readRemovedSkills, validateRemovals } from './removed-skills.mjs';
...
export function validateCapabilities(root) {
  ...
  // Deliberately removed optional skills have no folders; the removal record accounts for them.
  const removed = new Set(readRemovedSkills(root));
  errors.push(...validateRemovals(root, manifest, [...removed]));
  const checkFile = p => {
  ...
  return { errors, manifest, removed };
}
...
if (process.argv[1] && path.resolve(process.argv[1]) === ownPath) {
  try {
    ...
    const { errors, manifest, removed } = validateCapabilities(root);
    ...
    else console.log(`Skill capability checks passed (${Object.keys(manifest.skills).length - removed.size} active, ${removed.size ? `${removed.size} removed, ` : ''}${Object.keys(manifest.retired).length} retired).`);
