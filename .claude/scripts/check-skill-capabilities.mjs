...
import { printWarnings, readRemovedSkills, reviewRemovals } from './removed-skills.mjs';
...
if (process.argv[1] && path.resolve(process.argv[1]) === ownPath) {
  try {
    ...
    else console.log(`Skill capability checks passed (${Object.keys(manifest.skills).length - removed.size} active, ${removed.size ? `${removed.size} removed, ` : ''}${Object.keys(manifest.retired).length} retired${warnings.length ? `, ${warnings.length} warning(s)` : ''}).`);
