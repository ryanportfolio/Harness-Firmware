...
const standalone = JSON.parse(read(".agents/skill-sources.json")).standalone;
if (!Array.isArray(standalone) || standalone.some((name) => typeof name !== "string" || !/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(name)) || new Set(standalone).size !== standalone.length) {
  throw new Error("Invalid standalone Codex skill registry");
}
...
const expectedNames = [...new Set([...canonicalNames, ...standalone])];
const actualNames = fs.readdirSync(path.join(root, ".agents/skills"), { withFileTypes: true })
  .filter((entry) => entry.isDirectory() && exists(`.agents/skills/${entry.name}/SKILL.md`))
  .map((entry) => entry.name);
for (const name of actualNames) {
  if (!expectedNames.includes(name)) failures.push(`${name}: unregistered Codex skill`);
}
const skills = expectedNames.map((name) => {
    const canonicalPath = `.claude/skills/${name}/SKILL.md`;
    if (exists(canonicalPath)) {
      const canonicalMetadata = frontmatter(canonicalPath);
      if (canonicalMetadata.name && canonicalMetadata.name !== name) {
        failures.push(`${canonicalPath}: declared name ${canonicalMetadata.name} does not match directory ${name}`);
      }
    }
    const relativePath = `.agents/skills/${name}/SKILL.md`;
    if (!exists(relativePath)) {
      failures.push(`${relativePath}: missing Codex skill`);
      return { directory: name, name, description: "" };
    }
    const metadata = frontmatter(relativePath);
    if (metadata.name !== name) {
      failures.push(`${relativePath}: declared name ${metadata.name} does not match directory ${name}`);
    }
    if (standalone.includes(name) && (read(relativePath).includes("<!-- Generated by") || read(relativePath).includes(`.claude/skills/${name}/SKILL.md`))) {
      failures.push(`${relativePath}: standalone workflow must not be a Claude adapter`);
    }
    return {
      directory: name,
