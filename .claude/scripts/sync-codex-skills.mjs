...
// These skills are maintained directly in .agents/skills, not generated.
const standaloneSkills = new Set(["long-horizon"]);
...
if (fs.existsSync(sourceRoot)) {
  for (const entry of fs.readdirSync(sourceRoot, { withFileTypes: true })) {
    ...
    if (standaloneSkills.has(entry.name)) {
      const nativePath = path.join(targetRoot, entry.name, "SKILL.md");
      ...
      const nativeMetadata = readMetadata(entry.name, nativePath);
      if (nativeMetadata.name !== entry.name) {
        throw new Error(`${nativePath}: name must match ${entry.name}`);
      }
      desired.set(entry.name, null);
      continue;
    }
