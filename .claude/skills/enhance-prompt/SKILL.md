---
description: Use when the user asks to rewrite a rough request into a polished, copy/paste-ready prompt for another agent or a fresh session, or asks to "brief a new session" / write a "handoff prompt".
---
...
Not this skill: an independent verification or audit prompt for a fresh session → `handoff-audit`.

## 0: Source of the request

- `$ARGUMENTS` non-empty → that is the request.
- `$ARGUMENTS` empty, or "turn this into a prompt" / "hand this off" → the request is the current task in this conversation, or the follow-up task the user just named. Harvest what the session already established: file paths and line numbers found, decisions the user made, approaches tried and failed, open questions. These are facts, not leaks; they go in as plain statements ("The retry loop in `api/client.ts:88` swallows the timeout error"), never as references to the conversation ("as we saw"). Facts from before a later edit may be stale; recheck the ones the prompt leans on.
- Args empty and no task in session → ask what the prompt is for. Never invent one.
- Whenever args were empty, open the reply with one line stating what you took the request to be, so a wrong guess costs one exchange, not a session.
...
- **No invented facts.** User didn't say it, session didn't establish it, can't verify → `<TODO: user fills in>`, never fabricate.
...
Craft rules for prompt body. Conditional: apply only when the task calls for them.
...
- **Why on non-obvious constraints.** "Never use ellipses; the output is read by a text-to-speech engine" → receiver generalizes correctly. Bare rule → literal-minded misfires. Obvious constraint needs no why.
- **Calm imperative tone.** No "CRITICAL:", "YOU MUST", ALL-CAPS; modern models overtrigger on aggressive language. Plain "Do X" followed just as reliably. Strong emphasis ≤ 1 genuinely blocking rule.
- **Quality modifiers when quality is the point.** Above-and-beyond wanted → say concretely: "Include as many relevant features and interactions as possible; go beyond the basics." Vague → median output.
- **Structure only when content types mix.** Instructions + pasted logs/data/examples → wrap each (`<instructions>`, `<context>`, `<input>`, `<example>`) so data ≠ directive, and put 1k+ tokens of pasted material above the task. Format-sensitive deliverable → 1-2 short `<example>` tags. Role line only if it changes behavior ("senior security engineer reviewing for OWASP Top 10"). Short single-purpose prompt → none of this.
- **Self-check for verifiable work.** Phrase as stopping condition: "Don't finish until [the failing test passes / the type-check is clean / the listed criteria are met]". Match check to task; no generic "double-check your work".
...
- **Default skeleton for non-trivial prompts.** Context → task → constraints → verification → output format. Cold receiver scans labeled sections faster than interleaved prose. Short/simple task → skip.
...
Safe mechanical work (typo fix, rename one internal variable, add console log) → single phase. User explicitly asked for direct implementation → single phase, encode their choice.

Otherwise, doubt → phase. Skipped phase cheap; bad edit expensive to undo.
...
- **Escape hatch.** Receiver hits genuine blocker (missing access, contradictory requirement, `<TODO>` unresolved) → stop and ask, not guess. One line: "If anything here is ambiguous or blocked, ask before proceeding rather than guessing."
- **Report back.** User reviews the receiver's session cold. One line: "End with a short report: what changed (files), what was verified and how, what remains open or was skipped, and any placeholder you could not resolve." Skip for one-line mechanical tasks.
...
- **Known pitfalls for the touched area.** One line each, only if the task can hit them
...
Draft done → reread once as the cold receiver: would a colleague with no context on this project be confused anywhere → rewrite that line. Then per sentence: changes receiver behavior? No → cut. Catch conversation-context leaks ("as discussed", "the file we looked at"); receiver has none. Last: AI-tell scan (em dashes, AI vocabulary, caps shouting, filler); the receiver copies your register.
...
Above block: 1-2 sentence note on what changed from user's input, e.g., "Added a phase split, three file path references, and the copy-policy constraint since this touches user-facing wording."
...
- **No invented constraints.** Typo fix needs no 5-phase audit, XML tags, examples, or role line. Prompt complexity = job complexity.
...
- **No rule-file dumps.** Only task-relevant rules, as plain constraints.
- **No overriding user's clear choices.** Decided → encode as-is. Left open → ask or flag `<TODO>`.
- **No placeholders for verifiable facts.** Can grep the path / read the code / already established in session → write it, don't write `<file path>`.
